  ...
  module Search
    ...
    def search(order:)
      <<-SQL
        ...
        LEFT JOIN LATERAL (SELECT sign_activities.sign_id, COUNT(sign_activities.sign_id) AS count
                           FROM sign_activities
                           WHERE sign_activities.sign_id = sign_search.id AND sign_activities.key = 'agree'
                           GROUP BY sign_activities.sign_id) AS activity ON activity.sign_id = signs.id

module Admin
  class UserSignTransfersController < Admin::ApplicationController
    def new
      redirect_to root_path, alert: t(".old_owner_required") if old_owner.nil?
    end
    ...
    def old_owner
      @old_owner ||= User.find_by(id: user_sign_transfer_params[:old_owner])
    end

    def new_owner
      @new_owner ||= User.find_by(id: user_sign_transfer_params[:new_owner])
    end
  end
end
